...
class LineageStore {
  ...
  Future<void> initialize() async {
    ...
    db.execute(
      'CREATE INDEX IF NOT EXISTS pg_trace_events_trace_seq ON pg_trace_events(trace_id, seq);',
    );
    db.execute(
      'CREATE INDEX IF NOT EXISTS pg_embeddings_chunk_representation ON pg_embeddings(chunk_id, representation_type);',
    );
    db.execute(
      'CREATE INDEX IF NOT EXISTS pg_candidates_trace_lane_strategy ON pg_candidates(trace_id, lane, strategy_id);',
    );
    db.execute(
      'CREATE INDEX IF NOT EXISTS pg_rerank_features_trace_lane_strategy ON pg_rerank_features(trace_id, lane, strategy_id);',
    );
    db.execute(
      'CREATE INDEX IF NOT EXISTS pg_build_jobs_document_status ON pg_build_jobs(document_id, status);',
    );
    db.execute(
      'CREATE INDEX IF NOT EXISTS pg_vector_index_entries_embedding ON pg_vector_index_entries(embedding_id, strategy_id, lane);',
    );
    _ensureColumn(
      db,
      'pg_experiment_runs',
    ...
    _ensureColumn(db, 'pg_experiment_runs', 'failure_detail', 'TEXT');
    _initialized = true;
  }
  ...
  Future<void> replaceImportLineage(LineageImportResult result) async {
    ...
    try {
      ...
      db.execute('DELETE FROM pg_lineage_sections WHERE document_id = ?', [
        document.documentId,
      ]);
      db.execute('DELETE FROM pg_lineage_chunks WHERE document_id = ?', [
        document.documentId,
      ]);
  ...
  Future<LineageDocumentRecord?> lineageDocumentById(String documentId) async {
    await initialize();
  ...
  Future<List<LineageEmbedding>> embeddingsForChunk(String chunkId) async {
    ...
    return _db!
        .select(
          'SELECT * FROM pg_embeddings WHERE chunk_id = ? ORDER BY representation_type, embedding_id',
          [chunkId],
        )
  ...
  Future<List<LineageTrace>> latestTraces({int limit = 200}) async {
    ...
    return _db!
        .select(
          'SELECT * FROM pg_traces ORDER BY COALESCE(completed_at, started_at) DESC, trace_id DESC LIMIT ?',
          [safeLimit],
        )
        .map(_traceFromRow)
        .toList(growable: false);
  ...
  Future<List<TraceEventRecord>> eventsForTrace(String traceId) async {
    ...
    return _db!
        .select(
          'SELECT * FROM pg_trace_events WHERE trace_id = ? ORDER BY seq ASC',
          [traceId],
        )
        .map(_eventFromRow)
        .toList(growable: false);
  ...
  Future<List<CitationRecord>> citationsForTrace(String traceId) async {
    ...
    return _db!
        .select(
          'SELECT * FROM pg_citations WHERE trace_id = ? ORDER BY citation_id',
          [traceId],
        )
  ...
  Future<ExperimentRunRecord?> experimentRunById(String experimentRunId) async {
    await initialize();
    final rows = _db!.select(
      '''
  ...
  Future<List<BuildJobRecord>> buildJobsForStrategy(String strategyId) async {
    await initialize();
    return _db!
        .select(
          '''
  ...
  Future<void> pruneCompletedTraces({int keep = 200}) async {
    ...
    try {
      for (final row in rows) {
        ...
        db.execute(
          "DELETE FROM pg_embeddings WHERE source_kind = 'query' AND source_id = ?",
          [traceId],
        );
  ...
  LineageDocumentRecord _lineageDocumentFromRow(Row row) =>
      LineageDocumentRecord(
        ...
        extractedCharCount: (row['extracted_char_count'] as num).toInt(),
  ...
  LineageSectionRecord _lineageSectionFromRow(Row row) => LineageSectionRecord(
  ...
  LineageChunkRecord _lineageChunkFromRow(Row row) => LineageChunkRecord(
    ...
    overlapFromPrevious: (row['overlap_from_previous'] as num).toInt(),
  ...
  VectorIndexEntryRecord _vectorEntryFromRow(Row row) => VectorIndexEntryRecord(
    ...
    commitStatus: VectorCommitStatus.values.firstWhere(
      ...
      orElse: () => throw StateError(
        'Unknown vector commit status: ${row['commit_status']}',
      ),
    ),
  ...
  RerankFeatureRecord _rerankFeatureFromRow(Row row) => RerankFeatureRecord(
    ...
    candidateId: row['candidate_id'] as String,
    chunkId: row['chunk_id'] as String,
    normalizedLexicalAffinity: (row['normalized_lexical_affinity'] as num)
        .toDouble(),
    cosine: (row['cosine'] as num).toDouble(),
    dualChannelAgreement: (row['dual_channel_agreement'] as num).toDouble(),
    queryWindowCoverage: (row['query_window_coverage'] as num).toDouble(),
