...
void main() {
  test('active retrieval executes FTS, one query embedding, vector search, fusion and lineage',
      () async {
    ...
    expect(bundle.semanticHits.map((hit) => hit.chunk.id), <String>['c2', 'c1']);
    expect(bundle.hybridHits.first.chunk.id, 'c1');
    expect(bundle.hybridHits.first.channels, containsAll(<String>['fts5', 'embedding']));
    ...
    expect(events.map((event) => event.kind), containsAll(<String>[
