...
void main() {
  test(
    'local real-corpus benchmark cases survive a database restart',
    () async {
  ...
  test(
    'a local case requires an explicit expected document or chunk',
    () async {
  ...
  test('router and citation metrics stay unavailable without observations', () {
    ...
    final unavailable = evaluator.aggregate(
      const [benchmark],
      const [
        BenchmarkCaseResult(
          caseId: 'q-local',
    ...
    final observed = evaluator.aggregate(
      const [benchmark],
      const [
        BenchmarkCaseResult(
          caseId: 'q-local',
