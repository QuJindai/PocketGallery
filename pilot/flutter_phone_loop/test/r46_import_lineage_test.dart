...
class TestSemanticStore extends SemanticStore {
  TestSemanticStore(super.lexicalStore, VectorObservationStore observationStore)
...
void main() {
  test(
    'new markdown import records exact heading and chunk provenance',
    () async {
      ...
      for (final lineageChunk in result.chunks) {
        ...
        expect(
          source.substring(lineageChunk.startOffset!, lineageChunk.endOffset!),
          chunk.text,
  ...
  test(
    'vector migration never downgrades exact import lineage to legacy',
    () async {
  ...
  test(
    'KnowledgeEngine commits lexical and exact lineage before vector work',
    () async {
      ...
      final storedDocument = await lineage.lineageDocumentById(
        document.documentId,
      );
      ...
      final job = await lineage.buildJobById(
        LineageIds.buildJobId(
          document.documentId,
