...
void main() {
  test(
    'integrated R4.6/R4.7 release advances identity and CI guards',
    () async {
      ...
      expect(phoneWorkflow, contains('workflow_dispatch:'));
      expect(phoneWorkflow, isNot(contains('pull_request:')));
      ...
      expect(phoneWorkflow, contains('android-arm64 --split-per-abi'));
      expect(
        r46Workflow,
    ...
    for (var index = 0; index < requiredKinds.length; index++) {
      ...
      await store.appendEvent(
        TraceEventRecord(
          ...
          kind: kind,
          truthKind: kind == 'fusion.completed' || kind == 'context.budgeted'
              ? TruthKind.derived
              : TruthKind.real,
  ...
  test(
    ...
    () async {
      ...
      expect(
        golden,
        contains('lineageRecorder: engine.runtimeLineageRecorder'),
      );
      expect(golden, contains('lineageTraceId = reply.traceId'));
      expect(
        golden,
        isNot(contains("GateResult('F11_CITATION_LINEAGE', true")),
      );
    },
  );
}
