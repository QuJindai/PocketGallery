...
void main() {
  const chunk = PgChunk(
    id: 'c1', documentId: 'd1', sourceName: 'doc.txt', locator: 'text', ordinal: 0,
  ...
  test('auto retrieval accepts dual channel evidence', () {
    final bundle = RetrievalBundle(
      ...
      hybridHits: const [HybridHit(
        chunk: chunk, score: 0.02, channels: {'fts5', 'embedding'},
        lexicalRank: 1, semanticRank: 1,
      )],
  ...
  test('auto retrieval rejects weak single channel noise', () {
    final bundle = RetrievalBundle(
      lexicalHits: const [], semanticHits: const [],
      hybridHits: const [HybridHit(
        chunk: chunk, score: 0.018, channels: {'fts5'},
        lexicalRank: 1, semanticRank: null,
      )],
  ...
  test('semantic scope oversamples before document filtering', () async {
    ...
    expect(source.indexOf('searchSimilar'), lessThan(source.lastIndexOf('scope.documentIds')));
  });
}
