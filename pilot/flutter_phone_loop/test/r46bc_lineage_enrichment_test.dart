...
void main() {
  test('lineage source records resolve directly by chunk and section id',
      () async {
  ...
  test('candidate and evidence reads are isolated by strategy and lane',
      () async {
    final db = sqlite3.openInMemory();
    addTearDown(db.close);
    final store = await _storeWithTrace(db);
  ...
  test('experiment run progress and result survive a store round trip',
      () async {
    final db = sqlite3.openInMemory();
    addTearDown(db.close);
    final store = await _storeWithTrace(db);
