...
void main() {
  ...
  test(
    'evidence pack is conservative instead of blindly injecting five chunks',
    () {
      const chunks = [
        PgChunk(
          id: 'c1',
  ...
  test(
    'ordinary evidence remains capped at the first five qualifying hits',
    () {
      final hits = [
        for (var i = 0; i < 7; i++)
          HybridHit(
  ...
  test(
    ...
    () async {
      ...
      expect(
        cases.any(
          (c) => (c['tags'] as List<dynamic>).contains('phone-realworld'),
        ),
        isTrue,
      );
    },
  ...
  test(
    'Auto retrieval decision is visible in chat instead of only Knowledge ON',
    () async {
