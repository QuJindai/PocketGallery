...
void main() {
  ...
  test('distributable APK signing identity is pinned and cache miss fails closed',
      () async {
  ...
  test('next pilot build is a monotonic in-place update over R4.1', () async {
    ...
    final match = RegExp(r'^version:\s*[^+]+\+(\d+)\s*$', multiLine: true)
        .firstMatch(pubspec);
  ...
  test('canceled model downloads recover automatically without OAuth reset',
      () async {
    final setup =
