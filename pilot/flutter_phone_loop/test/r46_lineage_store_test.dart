...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(
        (await store.eventsForTrace(traceId)).single.kind,
        'trace.started',
      );
      ...
      expect(
        (await store.candidatesForTrace(traceId)).single.embeddingId,
        'emb-1',
      );
      ...
      expect(
        (await store.routerDecisionForTrace(
          traceId,
          strategy,
      ...
      expect(
        (await store.promptBudgetForTrace(traceId))!.remainingTokens,
        5842,
      );

      await store.putGenerationStats(
      ...
      expect(
        (await store.generationStatsForTrace(traceId))!.generationMs,
        1500,
      );

      await store.putCitation(
        CitationRecord(
          citationId: 'cit-1',
          traceId: traceId,
          anchor: 'E1',
      ...
      expect(
        (await store.citationsForTrace(traceId)).single.documentId,
        'doc-1',
      );
