...
void main() {
  test('one Auto knowledge turn persists the complete R4.6 lineage lifecycle',
      () async {
    ...
    final user = messages.singleWhere((message) => message.role == ChatRole.user);
  ...
  test('model failure keeps retrieval evidence and marks the same trace failed',
    ...
    await expectLater(
      fixture.orchestrator.sendMessage(
        session.id,
        '端侧模型性能测试如何保留血缘',
      ),
    ...
    expect(await fixture.store.candidatesForTrace(traces.single.traceId),
        isNotEmpty);
    expect(await fixture.store.evidenceForTrace(traces.single.traceId),
        isNotEmpty);
