...
void main() {
  ...
  test(
    'R4.0.2 chat uploads import into knowledge base and bind current session',
    () async {
  ...
  test(
    'R4.0.2 attachment switches pure-model chat to auto knowledge mode',
    () async {
      final chat = await File('lib/ui/chat_page.dart').readAsString();
  ...
  test(
    'R4.0.2 attachment chips detach from chat without deleting library file',
    () async {
      final chat = await File('lib/ui/chat_page.dart').readAsString();
