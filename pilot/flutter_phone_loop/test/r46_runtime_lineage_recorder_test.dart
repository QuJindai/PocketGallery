...
void main() {
  test('events are ordered and a failed trace keeps prior evidence', () async {
    ...
    final recorder = RuntimeLineageRecorder(
      store: store,
      clock: () =>
          DateTime.utc(2026, 8, 29).add(Duration(microseconds: tick++)),
    );
  ...
  test(
    'candidate batches are capped per channel and cap metadata is recorded',
    () async {
      final db = sqlite3.openInMemory();
      addTearDown(db.close);
      final store = LineageStore(database: db);
  ...
  test(
    'default retention keeps 200 traces and never removes chunk embeddings',
    () async {
      final db = sqlite3.openInMemory();
      addTearDown(db.close);
      final store = LineageStore(database: db);
