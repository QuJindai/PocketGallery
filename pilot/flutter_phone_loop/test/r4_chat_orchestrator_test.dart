...
RetrievalBundle bundle({bool relevant = false, bool evidence = false}) {
  const chunk = PgChunk(id: 'c', documentId: 'd', sourceName: 'd.txt', locator: 'text', ordinal: 0, text: 'local evidence');
  final ev = evidence ? const [EvidenceItem(anchor: 'E1', chunk: chunk, score: 0.1)] : const <EvidenceItem>[];
  final hybrid = evidence ? const [HybridHit(chunk: chunk, score: 0.1, channels: {'fts5'}, lexicalRank: 1, semanticRank: null)] : const <HybridHit>[];
  return RetrievalBundle(lexicalHits: const [], semanticHits: const [], hybridHits: hybrid, evidence: ev, lexicalOnly: true, autoRelevantOverride: relevant);
}

Future<(ChatOrchestrator, ChatSessionStore, FakeRetriever, FakeModel, String)> fixture(ChatMode mode, RetrievalBundle b) async {
  ...
  final m = FakeModel();
  return (ChatOrchestrator(store: store, retriever: r, model: m), store, r, m, s.id);
}
...
void main() {
  ...
  test('auto falls back to pure model when evidence is irrelevant', () async {
    final (o, _, r, m, id) = await fixture(ChatMode.auto, bundle(evidence: true, relevant: false));
  ...
  test('auto injects relevant local evidence', () async {
    final (o, _, _, m, id) = await fixture(ChatMode.auto, bundle(evidence: true, relevant: true));
  ...
  test('second turn receives first user and assistant pair', () async {
    ...
    expect(m.lastPrior.map((x) => x.text).toList(), containsAllInOrder(['第一问', '模型回答']));
  });
}
