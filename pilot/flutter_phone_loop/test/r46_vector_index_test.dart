...
void main() {
  test('explicit vector adapter indexes by embedding id and searches the supplied query vector', () async {
    ...
    backend.nextHits = const [
      BackendVectorHit(
        id: 'emb-body-1',
        similarity: 0.81,
        metadataJson: '{"chunkId":"chunk-1","documentId":"doc-1","modelIdentity":"EmbeddingGemma-test"}',
      ),
    ];
  ...
  test('document scope is enforced after deterministic overfetch without changing the query vector', () async {
    final backend = RecordingVectorBackend();
    final index = SqliteActiveVectorIndex.forTest(backend);
    await index.initialize();
    const query = [0.4, 0.6];
    backend.nextHits = const [
      BackendVectorHit(
        id: 'emb-other',
        similarity: 0.95,
        metadataJson: '{"chunkId":"c-other","documentId":"doc-other","modelIdentity":"m"}',
      ),
      BackendVectorHit(
        id: 'emb-wanted',
        similarity: 0.88,
        metadataJson: '{"chunkId":"c-wanted","documentId":"doc-wanted","modelIdentity":"m"}',
      ),
    ];
  ...
  test('probe exposes initialization truth and R4.6 database identity without pretending search verification', () async {
    final backend = RecordingVectorBackend();
    ...
    expect(after.searchVerified, isFalse,
        reason: 'Task 2 must not fabricate the later Search Verified health gate');
  });
}
