...
void main() {
  ...
  test('R5.0 build 23 maps to candidate Android versionCode 2023', () async {
    ...
    final result = await Process.run('bash', <String>[
      'scripts/android_version_code.sh',
    ]);
  ...
  test(
    'committed manifests exactly match generated source snapshots',
    () async {
      for (final expectation in <String, String>{
        'inner': 'MANIFEST.sha256',
        'outer': '../../DROPIN_MANIFEST.sha256',
      }.entries) {
        final generated = await Process.run('bash', <String>[
  ...
  test(
    'both workflows compile the checked-out source commit into every APK',
    () {
      for (final workflow in <String>[tddWorkflow, canonicalWorkflow]) {
        expect(
          workflow,
          matches(RegExp(r'SOURCE_COMMIT="\$\(git rev-parse HEAD\)"')),
        );
        expect(
          workflow,
          contains(
  ...
  test(
    'TDD workflow publishes an explicit non-canonical R5 arm64 artifact',
    () {
      expect(
        tddWorkflow,
  ...
  test(
    'canonical workflow builds and verifies the same-source upgrade pair',
    () {
  ...
  test(
    'canonical evidence is emitted only after tests and pair verification',
    () {
      final analyze = canonicalWorkflow.indexOf('- name: Analyze');
      final tests = canonicalWorkflow.indexOf('- name: Unit tests');
      final builds = canonicalWorkflow.indexOf(
        '- name: Build same-source R5.0',
      );
  ...
  test('canonical signing is isolated from pull requests and Actions cache', () {
    ...
    expect(canonicalWorkflow, isNot(contains('pocketgallery-r3-signing-v1')));
    expect(canonicalWorkflow, isNot(contains('feature/phone-pilot-')));
    expect(
      canonicalWorkflow,
    ...
    expect(
      ...
      isNot(
        matches(
          RegExp(r'keytool\s+-(?:genkey|genkeypair)', caseSensitive: false),
        ),
      ),
    );
