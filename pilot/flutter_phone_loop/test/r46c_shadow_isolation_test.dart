...
void main() {
  test(
    'on-demand shadow run reuses query vector and cannot mutate ACTIVE',
    () async {
  ...
  test(
    'shadow failure remains inspectable and leaves trace complete',
    () async {
      final lineageDb = sqlite3.openInMemory();
      final lexicalDb = sqlite3.openInMemory();
      addTearDown(lineageDb.close);
      ...
      expect(
        (await data.store.traceById('tr-shadow'))!.status,
  ...
  test('shadow ignores same-dimension embeddings from a stale model', () async {
    final lineageDb = sqlite3.openInMemory();
    final lexicalDb = sqlite3.openInMemory();
    addTearDown(lineageDb.close);
    addTearDown(lexicalDb.close);
    ...
    await data.lexical.replaceDocument(
      const ImportedDocument(
        documentId: 'd-stale',
        sourceName: 'stale.md',
        sha256: 'sha-stale',
  ...
  test(
    'shadow fails closed when the experiment model differs from query',
    () async {
      final lineageDb = sqlite3.openInMemory();
      final lexicalDb = sqlite3.openInMemory();
      addTearDown(lineageDb.close);
